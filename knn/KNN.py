...
# Для каждой точки тестовых данных делаем следующее:
# 1. Рассчитываем расстояние между данными испытаний и каждой строкой тренировочных данных с помощью любого из методов (в нашем случае Евклидово расстояние)
# 2. Теперь, основываясь на значении расстояния, сортируем в порядке возрастания.
# 3. Далее выбираем верхние K строк из отсортированного массива.
# 4. Теперь назначаем класс контрольной точке на основе наиболее часто встречающегося класса в этих строках.
...
def KNN_classify(train_data, test_data, k, number_of_classes):
    ...
    for test_point in test_data:
        ...
        test_dist = [
            [distance(test_point[0], test_point[1], train_data[i][0][0], train_data[i][0][1]), train_data[i][1]] for i
...
def show_data_on_mesh_after_classify(n_classes, n_items_in_class, k):
    ...
    def generate_test_mesh(train_data):
        x_min = min([train_data[i][0][0] for i in range(len(train_data))]) - 1.0
        x_max = max([train_data[i][0][0] for i in range(len(train_data))]) + 1.0
        y_min = min([train_data[i][0][1] for i in range(len(train_data))]) - 1.0
        y_max = max([train_data[i][0][1] for i in range(len(train_data))]) + 1.0
...
def calculate_accuracy(n_classes, n_item_in_class, k, test_percent):
    ...
    train_data, testDataWithLabels = split_train_test(data, test_percent)
    test_data = [testDataWithLabels[i][0] for i in range(len(testDataWithLabels))]
    testDataLabels = KNN_classify(train_data, test_data, k, n_classes)
    print("Accuracy: ",
